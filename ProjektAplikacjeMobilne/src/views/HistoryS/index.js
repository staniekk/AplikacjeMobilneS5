...
const HistoryS = () => {
  ...
  const getData = async () => {
    axios
      .get(
        `https://65a40329a54d8e805ed451eb.mockapi.io/api/am/history?userID=${userID}`
      )
      .then((response) => {
        setHasHistory(true);
      ...
      {history.map((entry) => (
        <View key={entry.id} style={styles.historyInfo}>
          <View key={entry.id} style={styles.historyInfo2}>
            <Image
              style={styles.runner}
              source={require("../../img/logo/runner.png")}
            />
            <View style={{ flex: 1 }}>
    ...
    <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
      <View style={styles.historyInfo}>
        <View style={styles.historyInfo2}>
          <Text style={styles.historyInfoText}>You have no history</Text>
        </View>
      </View>
    </View>
  );
